...
def __shapelet_candidate_loss(cand, time_series_set, label, num_segment, seg_length,
    ...
    while cnt < max_iters:
        ...
        for x, y in dataloader:
            ...
            # print('dist_tensor before', dist_tensor)
            for k in range(x.shape[0]):
                tmp_dist = pattern_distance_torch(
                ...
                # print('pattern_distance_torch', tmp_dist)
                dist_tensor[k] = tmp_dist
            # print('dist_tensor later', dist_tensor)
            ...
            # print('transformed dist_tensor', dist_tensor)
            ...
            # print(lb_idx)
            for k in range(0, len(lb_idx)):
                for j in range(k+1, len(lb_idx)):
                    src = dist_tensor[lb_idx[k]]
                    dst = dist_tensor[lb_idx[j]]
                    # print('normal')
                    # print(dist_tensor.shape)
                    # print(src.shape)
                    # print(dst.shape)
                    # print(torch.mean(src), torch.std(src, unbiased=False))
                    # print(Normal(torch.mean(src), torch.std(src, unbiased=False)))
                    # print(Normal(torch.mean(dst), torch.std(dst, unbiased=False)))
                    loss -= torch.abs(torch.distributions.kl.kl_divergence(
                        Normal(torch.mean(src), torch.std(src, unbiased=False)),
                        Normal(torch.mean(dst), torch.std(dst, unbiased=False))))
